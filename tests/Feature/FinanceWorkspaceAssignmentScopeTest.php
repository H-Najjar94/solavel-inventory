...
class FinanceWorkspaceAssignmentScopeTest extends TestCase
{
    ...
    public function test_solacount_only_member_passes_assignment_only_for_document_follow_through():void {
        foreach (\App\Services\InventoryWorkspace\FinanceDocumentLifecycleAuthority::ACTIONS as $action) $this->assertSame('passed_assignment_gate',$this->invoke($action),$action);
        foreach (['finance-allocations.reserve','finance-sources.receipts','items.index','workspace.context','workspace.initialize','warehouses.store'] as $action) {
