...
test('owner creates and removes an organization tax through the UI', async ({ page }) => {
    ...
    const taxForm = taxPanel.locator('form');
    await taxForm.getByLabel('Code', { exact: true }).fill('QA-VAT-20260719');
    await taxForm.getByLabel('Name', { exact: true }).fill('QA Standard VAT');
    await taxForm.getByRole('spinbutton', { name: 'Rate', exact: true }).fill('15');
    await taxForm.getByRole('button', { name: 'Add tax' }).click();
    await expect(taxPanel.getByLabel('Tax name QA-VAT-20260719')).toHaveValue('QA Standard VAT');
