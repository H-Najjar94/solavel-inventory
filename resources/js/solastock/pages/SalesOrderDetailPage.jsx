    ...
    return (
        <section className="page">
            ...
            {tab === 'lines' && <div className="panel"><table className="data-table">
                ...
                <tbody>{lines.map((l) => <tr key={l.id}><td>{l.item?.name ?? `#${l.item_id}`}{l.item?.sku && <span className="muted"> · {l.item.sku}</span>}</td><td>{l.entered_qty ?? l.ordered_qty}{l.entered_unit ? <span className="muted"> {l.entered_unit.code} ({l.ordered_qty} base)</span> : null}</td><td>{l.reserved_qty}</td><td>{l.picked_qty}</td><td>{l.packed_qty}</td><td>{l.shipped_qty}</td><td>{l.unit_conversion_factor ? (Number(l.unit_price) * Number(l.unit_conversion_factor)).toFixed(4) : l.unit_price}</td><td>{l.discount_amount ?? '0.00'}</td><td>{l.tax_amount ?? '0.00'}</td><td>{l.line_total ?? '0.00'}</td></tr>)}</tbody>
            </table></div>}
